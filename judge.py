...
def preprocessing(s):
    # s="猛暑が無い都市に来ているのは、もう、しょうがない歳にきているから。" #?????????????「人民のじんみんによる人民のための政治」みたいな書き方はあかん。数字、英語、記号は極力使わない。四則演算は使えない。alkanaの辞書に登録されていなかったりローマ字読みができない英語（TOTO,Instagram,Twitter,TikTokなど）も使えない。英数字は半角のみ。半角カナは使えない。????????????[15001500人]のコーチ、100匹の猿と高知去る 檻 on the オリオン座。SOS団とSNS団、-10個のMissは見捨てようMY JR
